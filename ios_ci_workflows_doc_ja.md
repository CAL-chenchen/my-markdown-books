...
### 3. AdHoc プロファイル更新 ワークフロー

**目的**: ビルド時に正しいプロビジョニングプロファイルが必要です。古いプロファイルだとビルドが失敗します

**設定**:
- **トリガー**: 手動実行
- **プロビジョニングプロファイル**: Ad-Hoc
- **典型的な使用例**:
  - 新しいテストデバイス（UUID経由）を追加した場合
  - 有効期限が切れる前に更新
  - Apple Developer Programのメンバー変更や証明書再発行

**ワークフローファイル**: `.github/workflows/update_adhoc.yml`

**主な機能**:
- Apple Developer アカウントでプロファイル更新
- fastlane matchで証明書・プロファイルをGit管理（暗号化）
...
6. TestFlightに送信
7. Firebase Crashlytics に dSYM をアップロード
    ↓
Appleがビルドを処理（約5-15分）
    ↓
...
以下のGitHub Secretsが使用されています：

### 認証
- `APP_IDENTIFIER`: iOS アプリの Bundle Identifier
- `APPLE_ID`: App Store Connect への 2FA ログイン に使用する Apple ID
- `APPLE_ID_SPECIAL_PASSWORD`: 2FA を有効にしており、App Store Connect にバイナリをアップロードする場合に必要なアプリケーション用パスワード（詳細は [application-specific-password](https://docs.fastlane.tools/getting-started/ios/authentication/#method-3-application-specific-passwords) を参照）
...
- `FIREBASE_SERVICE_ACCOUNT_JSON`: 「サービスアカウントキー」を JSON 形式で書いた中身（ファイルの内容）
- `FIREBASE_SERVICE_ACCOUNT_DEBUG_JSON`: サービスアカウントキーの JSON 形式 (debugバージョン)
...
- **タグ付け**: リリースには[セマンティックバージョニング](https://semver.org/lang/ja/)を使用（例：タイミー 25.12.17、ぺいぺい 5.33.0、メルカリ 5.182.1、YouTube 20.50.9）
...
**1. コード署名エラー**
```bash
# GitHub Actionsで証明書を再生成
bundle exec fastlane match appstore --force
bundle exec fastlane match adhoc --force
```

**2. Firebaseアップロード失敗**
- `FIREBASE_SERVICE_ACCOUNT_JSON`が有効か確認
...
- 証明書リポジトリはプライベート
- Matchリポジトリへのアクセスは制限
- .p8ファイルはリポジトリにコミットしない (App Store Connect API Keyのみ)
